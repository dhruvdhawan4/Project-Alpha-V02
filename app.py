...
# ==============================================================================
    ...
    with t1:
        ...
        c3.metric("P/E Ratio", f"{metrics.get('pe', 0.0):.2f}" if is_valid_number(metrics.get('pe')) else "N/A")
        c4.metric("P/B Ratio", f"{metrics.get('pb', 0.0):.2f}" if is_valid_number(metrics.get('pb')) else "N/A")
        
        c5, c6, c7, c8 = st.columns(4)
        c5.metric("ROE", f"{metrics.get('roe', 0.0)*100:.2f}%" if is_valid_number(metrics.get('roe')) else "N/A")
        c6.metric("Debt-to-Equity", f"{metrics.get('debt_to_equity', 0.0):.2f}" if is_valid_number(metrics.get('debt_to_equity')) else "N/A")
        c7.metric("Operating Margin", f"{metrics.get('operating_margins', 0.0)*100:.2f}%" if is_valid_number(metrics.get('operating_margins')) else "N/A")
        c8.metric("Inst. Holdings", f"{metrics.get('inst_ownership', 0.0)*100:.2f}%" if is_valid_number(metrics.get('inst_ownership')) else "N/A")
    ...
    with t2:
        if not df_hist.empty:
            ...
            m1.metric("RSI (14)", f"{last_row.get('RSI', 0.0):.2f}")
            m2.metric("MACD", f"{last_row.get('MACD', 0.0):.2f}")
            m3.metric("ADX (14)", f"{last_row.get('ADX', 0.0):.2f}")
            m4.metric("ATR (14)", f"{last_row.get('ATR', 0.0):.2f}")
