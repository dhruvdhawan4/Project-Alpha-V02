...
# ==============================================================================
    ...
    with t1:
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Sector", str(metrics.get("sector", "N/A")))
        c2.metric("Industry", str(metrics.get("industry", "N/A")))
        
        pe_val = metrics.get('pe')
        pb_val = metrics.get('pb')
        roe_val = metrics.get('roe')
        debt_val = metrics.get('debt_to_equity')
        margin_val = metrics.get('operating_margins')
        ...
        c6.metric("Debt-to-Equity", f"{float(debt_val):.2f}" if is_valid_number(debt_val) else "N/A")
        c7.metric("Operating Margin", f"{float(margin_val)*100:.2f}%" if is_valid_number(margin_val) else "N/A")
