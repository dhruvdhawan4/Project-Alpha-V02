"""
Institutional Visualizations & UI Render Components
"""
...
def is_valid_number(val: Any) -> bool:
    """Safely checks if a value is a valid numeric scalar."""
    if val is None:
        return False
    try:
        v = float(val)
...
def render_macro_header(header_data: Dict[str, Any]) -> None:
    """Renders real-time global index ticker widgets with market sentiment indicators."""
    st.markdown("""
        <style>
            .macro-card {
...
def plot_monte_carlo_paths(simulation_results: Dict[str, Any], currency_symbol: str = "$") -> go.Figure:
    """Plots visual trajectories for Monte Carlo iterations."""
    fig = go.Figure()
    paths = simulation_results.get("simulation_paths", [])
    
    for i in range(min(100, len(paths))):
        fig.add_trace(go.Scatter(y=paths[i], mode='lines', line=dict(width=1), opacity=0.12, showlegend=False))
        
    fig.update_layout(
        title="Monte Carlo Portfolio Simulation Trajectories (50,000 Runs Sampled)",
        xaxis_title="Trading Days",
        yaxis_title=f"Portfolio Projected Value ({currency_symbol})",
...
def plot_technical_indicators(df: pd.DataFrame, ticker: str) -> go.Figure:
    """Generates multi-pane technical chart including Price, Moving Averages, VWAP, and RSI."""
    fig = go.Figure()
    ...
    if 'SMA20' in df.columns:
        fig.add_trace(go.Scatter(x=df.get('Date', df.index), y=df['SMA20'], mode='lines', name='SMA 20', line=dict(color='orange', width=1)))
    if 'SMA50' in df.columns:
        fig.add_trace(go.Scatter(x=df.get('Date', df.index), y=df['SMA50'], mode='lines', name='SMA 50', line=dict(color='blue', width=1)))
    if 'VWAP' in df.columns:
        fig.add_trace(go.Scatter(x=df.get('Date', df.index), y=df['VWAP'], mode='lines', name='VWAP', line=dict(color='purple', width=1.5)))

    fig.update_layout(
        title=f"Technical Price Chart & Indicators - {ticker}",
