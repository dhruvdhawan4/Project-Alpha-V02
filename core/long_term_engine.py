...
class LongTermEngine:
    ...
    def analyze_universe(self, market: str, vehicle: str, investment_amount: float) -> pd.DataFrame:
        """Executes multi-factor analysis across asset class universe."""
        ...
        for symbol in tickers:
            ...
            ret_1y = (close[-1] - close[-252]) / close[-252] if len(close) >= 252 else 0.10
            ret_5y = (close[-1] - close[0]) / close[0] if len(close) >= 1250 else ret_1y * 3
