...
class LongTermEngine:
    ...
    def _compute_factor_weights(self, X: pd.DataFrame, y: pd.Series) -> Dict[str, float]:
        """Calculates factor weights statistically using Random Forest and Permutation Importance."""
        if X.empty or len(X) < 5:
            # Fallback uniform weights across factors
    ...
    def analyze_universe(self, tickers: List[str], market: str, vehicle: str) -> pd.DataFrame:
        ...
        for raw_symbol in tickers:
            ...
            # Composite Factor Metric Mapping
        ...
        # Dynamically compute weights using Sharpe Ratio as target y
        X = df_factors[[f"{c}_z" for c in factor_cols]]
        y = df_factors['sharpe']
        weights = self._compute_factor_weights(X, y)
        
        # Aggregate institutional score (0 - 100)
    ...
    def run_monte_carlo_simulation(
        ...
        return {
            ...
            "simulation_paths": portfolio_paths[:100, :]  # Sample 100 paths for UI plotting
        }
